...
class AndroidTaskManager:
    """
    Android任务管理器，包含任务队列和Android设备实例管理
    """
    ...
    def _get_device_id(self, kwargs: Dict[str, Any]) -> str:
        ...
        if not device_id:
            raise ValueError("任务参数中必须包含 device_id")
    ...
    def submit_task(self, task_func: Callable, **kwargs):
        ...
        :param kwargs: 传递给任务函数的参数，必须包含 device_id 或相关标识
        """
...
# 创建全局Android任务管理器实例
android_task_manager = AndroidTaskManager(max_concurrent=4)