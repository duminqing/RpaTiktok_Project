...
def execute_single_account_task(account_id):
    """
    为单个账号执行任务的方法
    ...
    """
    try:
        account = TikTokAccountInfo.objects.get(id=account_id, account_status=0)
    except TikTokAccountInfo.DoesNotExist:
        print(f"未找到ID为 {account_id} 且状态为0的账号")
        return
